...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (2073012000)

) I
) C UNION ALL 
SELECT 1 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37017346)
...
JOIN #Codesets cns on (po.procedure_source_concept_id = cns.concept_id and cns.codeset_id = 0)
) C
...
FROM #qualified_events E
JOIN @cdm_database_schema.PERSON P ON P.PERSON_ID = E.PERSON_ID
WHERE E.start_date >= DATEFROMPARTS(2016, 1, 1)
GROUP BY e.person_id, e.event_id
-- End Demographic Criteria
