...
root <- "J:/amarkus/Documents/Experiments_EXPLORE"

path <- file.path(root, "BenchmarkExplore", "output", "IPCI")
...
set.seed(3110)
population <- population[sample(1:nrow(population), min(max_obs, nrow(population))),]
...
x <- as.matrix(x)
colnames(x) <- 1:ncol(x) # name columns?
...
if (method == "univariate") {
  ...
  features <- x[,order(abs(correlation), decreasing = TRUE) <= n_var]
  ...
  features <- x[,names(coef_ordered[1:min(n_var, n_sel)])]
...
dataset <- data.frame(features,y)
colnames(dataset) <- c(colnames(features), 'class')

farff::writeARFF(dataset, paste0(root, "/", file_name, ".arff"))
